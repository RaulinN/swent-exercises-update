...
final class Vector3Test {
    ...
    public void testL() {
        ...
        // using the identity log(x * y) = log x + log y

        // Alice's implementation
        float z = (float) Math.log(s * s);
        Vector3 v1 = new Vector3(0, 1, z);

        // Bob's implementation
        float zHalf = (float) Math.log(s);
        Vector3 v2 = new Vector3(0, 1, zHalf * 2);

        assertThat(v1.x, is(v2.x));
        assertThat(v1.y, is(v2.y));
        assertThat(v1.z, is(v2.z));
    }
}
